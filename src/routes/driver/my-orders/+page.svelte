...
<h1 class="bg-blue-500 rounded-xl p-4 text-2xl cursor-default mt-5 text-white">My orders</h1>
...
<div>
  ...
  />
  <label for="declined-orders" class="cursor-pointer">Show my completed orders</label>
</div>
  ...
  {#if completedOrders}
    {#if data.completedOrders.length === 0}
      ...
      <br>Explore the <a class="font-bold hover:underline" href="/driver">new orders</a> and pick one!
      </p>
    {/if}
